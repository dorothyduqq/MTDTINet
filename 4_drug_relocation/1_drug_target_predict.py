...
evaluation_path = '../2_train_model_plus/evaluation/effect_targets.csv'
drug_path = '../1_data_preparation_plus/3_get_pos_datas/pos_pre_short_data.csv'
output_dir = './1_drug_target_predict/'
...
drug_df = pd.read_csv(drug_path).drop_duplicates(subset=['canonical_smiles'])
drug_df = drug_df[drug_df['canonical_smiles'].apply(is_valid_smiles)]
# drug_df = drug_df[drug_df['canonical_smiles'].str.len() <= 350]
...
drug_smiles = drug_df['canonical_smiles'].tolist()
...
# 按批次处理药物数据
for batch_idx in trange(start_batch, len(drug_smiles) // batch_size + 1):
    batch_start = batch_idx * batch_size
    batch_end = min((batch_idx + 1) * batch_size, len(drug_smiles))
    batch_smiles = drug_smiles[batch_start:batch_end]
    drug_batch = get_batch_drug_encodings(batch_smiles)
    ...
    for j, target_protein_id in enumerate(target_protein_ids):
        ...
        # 使用分批次预测函数
        avg_class, avg_reg = predict_in_batches(models, drug_batch, protein_batch, predict_batch_size)
